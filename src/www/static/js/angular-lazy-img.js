...
var app = angular.module('angularLazyImg', ["MusicApp"]);

app.factory('LazyImgMagic', [
...
app.provider('lazyImgConfig', function() {
  'use strict';
...
app.directive('lazyImg', [
    '$rootScope', '$log', 'LazyImgMagic', 'youtubeFuncs', function ($rootScope, $log, LazyImgMagic, youtubeFuncs) {
      'use strict';
      ...
      function link(scope, element, attributes) {
        ...
        // });
        var set = false;
        var watchKey = attributes.lazyImg;
        var unreg = scope.$watch(watchKey, function(newval, oldval, scope) {
          if (!set || newval.url != oldval.url) {
            set = true;
            scope.lazyImage.setSource(youtubeFuncs.getThumbnail(newval));
          }
        });
